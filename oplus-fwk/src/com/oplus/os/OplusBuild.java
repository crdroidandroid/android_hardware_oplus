...
public class OplusBuild {
    ...
    public static class VERSION {
        ...
        // *** DO NOT CHANGE ***
        // vendor apps depend on this API contract
    ...
    /**
     * This method MUST exist and return a valid Oplus OS SDK number.
     * Vendor apps like Brickmode & Camera expect it.
     */
    public static int getOplusOSVERSION() {

        // Try resolving dynamically from RELEASE string
        for (int i = VERSIONS.length - 2; i >= 0; i--) {
            if (!TextUtils.isEmpty(VERSION.RELEASE)
                    && VERSION.RELEASE.startsWith(VERSIONS[i])) {
                return i + 1;
            }
        }
